...
class WGAN(object):
    ...
    def _read_input(self):
        ...
        for f in filenames:
            ...
            while len(indices) > self.batch_size:
                im_batch = np.array([all_pairs[i][0] for i in indices[-self.batch_size:]])
                triple_batch = np.array([all_pairs[i][1] for i in indices[-self.batch_size:]])
                #Convert to one hot
                t_batch = np.zeros((self.batch_size, 3, self.vocab_size))
    ...
    def create_network(self, optimizer="Adam", learning_rate=1e-4,
        ...
        #self.actual_graphs is a placeholder for the current batch of actual scene graphs
        logits_real = self._discriminator(self.image_feats, self.real_graphs, scope_name="discriminator")

        #self.image_feats is a placeholder for the current batch of image feats
        #self.actual_graphs is the current batch of generated scene graphs
