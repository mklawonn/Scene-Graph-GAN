...
class Generator(object):
    ...
    def __init__(self, vocab_size, dim_context=[196, 512], seq_len = 3, dim_hidden=512, batch_size=64):
        ...
        self.att_W = tf.get_variable("att_W", [dim_hidden, dim_context[0]], initializer = he_initializer)
        self.att_b = tf.get_variable("att_b", [dim_context[0]], initializer = constant_initializer)
        ...
        self.sequence_W = tf.get_variable("sequence_W", [dim_context[1] + dim_hidden, dim_context[1]*seq_len], initializer = he_initializer)
        self.sequence_b = tf.get_variable("sequence_b", [dim_context[1]*seq_len], initializer = constant_initializer)
    ...
    def build_generator(self, context, batch_size, gumbel_temp):
        ...
        z_hat_and_noise = tf.concat([embedded_context, embedded_noise], axis=1)
        #resnet_input = tf.add(tf.matmul(z_hat, self.sequence_W), self.sequence_b)
        resnet_input = tf.add(tf.matmul(z_hat_and_noise, self.sequence_W), self.sequence_b)
